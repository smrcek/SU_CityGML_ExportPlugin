require 'su_citygmlbridges\Export\WorldFileLoader'
require 'su_citygmlbridges\Export\GMLExportDialog'
require 'su_citygmlbridges\Attribute/attribute'
require 'su_citygmlbridges\UI\ProgressBar'
...
module FHGelsenkirchenBridges
  ...
  class CityGMLExport
    ...
    def exportGmlCoreAttributes(dictionaries)
      ...
      if(stdDict != nil)
        ...
        available.each do |key|
          ...
          value = FHGelsenkirchenBridges::stringDecode(stdDict[key]);
          if(value == nil)
            next;
          end
          case key
    ...
    def exportGenAttributes(dictionaries)
      ...
      if(genDict != nil)
        genDict.each do |k,v|
          key = FHGelsenkirchenBridges::stringDecode(k);
          value = FHGelsenkirchenBridges::stringDecode(v);
      ...
      if(stdDict != nil)
        ...
        available.each do |key|
          ...
          case key
          when "measuredHeight"
            @handle << "<brid:measuredHeight uom=\"m\">#{value.to_s()}</brid:measuredHeight>\n"
          when "storeysAboveGround"
            @handle << "<brid:storeysAboveGround>#{value.to_s()}</brid:storeysAboveGround>\n"
          when "storeysBelowGround"
            @handle << "<brid:storeysBelowGround>#{value.to_s()}</brid:storeysBelowGround>\n"
          when "class"
            @handle << "<brid:class>#{value.to_s()}</brid:class>\n"
          when "function"
            @handle << "<brid:function>#{value.to_s()}</brid:function>\n"
          when "usage"
            @handle << "<brid:usage>#{value.to_s()}</brid:usage>\n"
          when "yearOfConstruction"
            @handle << "<brid:yearOfConstruction>#{value.to_s()}</brid:yearOfConstruction>\n"
          when "yearOfDemolition"
            @handle << "<brid:yearOfDemolition>#{value.to_s()}</brid:yearOfDemolition>\n"
          when "storeyHeightsAboveGround"
            @handle << "<brid:storeyHeightsAboveGround uom=\"m\">#{value.to_s()}</brid:storeyHeightsAboveGround>\n"
          when "storeyHeightsBelowGround"
            @handle << "<brid:storeyHeightsBelowGround uom=\"m\">#{value.to_s()}</brid:storeyHeightsBelowGround>\n"
          when "roofType"
            @handle << "<brid:roofType>#{value.to_s()}</brid:roofType>\n"
			when "description"
            @handle << "<gml:description>#{value.to_s()}</gml:description>\n"
          when "name"
            @handle << "<gml:name>#{value.to_s()}</gml:name>\n"
            ...
            if(!isbatch)
              ...
              files.each do |skpN,xmlN|
                if(Sketchup.open_file(skpN))
                  ...
                  @filename_pur = FHGelsenkirchenBridges::str_filter(@filename_pur)
                  export(xmlFileName)
                end
              end
            end
...
## doplnena funkce dle pozadovaneho typu
      ...
      @handle << "<!-- Exported with Google SketchUp CityGML-Plugin < version #{FHGelsenkirchenBridges::CITYGML_VERSION} Build #{FHGelsenkirchenBridges::CITYGML_BUILD} > - #{FHGelsenkirchenBridges::CITYGML_CREATOR} -->\n"
      #Header für CityGML 1.0
      @handle << "<core:CityModel xmlns:xsi=\"http://www.w3.org/2001/XMLSchema-instance\" xsi:schemaLocation=\"http://www.opengis.net/citygml/2.0 ./CityGML_2.0/CityGML.xsd\" xmlns=\"http://www.opengis.net/citygml/2.0\" xmlns:xAL=\"urn:oasis:names:tc:ciq:xsdschema:xAL:2.0\" xmlns:app=\"http://www.opengis.net/citygml/appearance/2.0\" xmlns:wtr=\"http://www.opengis.net/citygml/waterbody/2.0\" xmlns:gen=\"http://www.opengis.net/citygml/generics/2.0\" xmlns:luse=\"http://www.opengis.net/citygml/landuse/2.0\" xmlns:tran=\"http://www.opengis.net/citygml/transportation/2.0\" xmlns:frn=\"http://www.opengis.net/citygml/cityfurniture/2.0\" xmlns:veg=\"http://www.opengis.net/citygml/vegetation/2.0\" xmlns:tun=\"http://www.opengis.net/citygml/tunnel/2.0\" xmlns:tex=\"http://www.opengis.net/citygml/textures/2.0\" xmlns:core=\"http://www.opengis.net/citygml/base/2.0\" xmlns:gml=\"http://www.opengis.net/gml\" xmlns:brid=\"http://www.opengis.net/citygml/bridge/2.0\" xmlns:dem=\"http://www.opengis.net/citygml/relief/2.0\" xmlns:bldg=\"http://www.opengis.net/citygml/building/2.0\">\n"
        ...
        if(entity.class ==Sketchup::Group or entity.class == Sketchup::ComponentInstance)
          ...
          begin
            ...
            t.each do |t2|
              t2.each_pair { | key, value |
                if(key == "id" || key == "ID")
                  buildingid = FHGelsenkirchenBridges::stringDecode(value.to_s())
                  #ID schon exportiert?
          ...
          if(proceed or nosurfacetype != "")
            ...
            if(buildingid != nil and buildingid != "" )
              if(@exportedid[buildingid] == nil)
                ...
                @handle << "<brid:Bridge gml:id=\"" + buildingid.to_s() + "\">\n"
                ...
                @handle << "<brid:Bridge gml:id=\"" + actbuildingid.to_s() + "\">\n"
          ...
          if(stdbuilding == "")
            if(@generate)
              ...
              stdbuilding << "<brid:Bridge gml:id=\""+ stdbuildingid.to_s()+ "\">\n"
              @exportedid[stdbuildingid.to_s()] = 1
              puts "Building \"#{stdbuildingid}\" exported"
            else
              stdbuilding << "<core:cityObjectMember>\n"
              stdbuilding << "<brid:Bridge>\n"
            end
          end
    ...
    def writebuffers(lod, nosurfacetype, actbuildingid, readLayers)## doplneny pozadovany typ
      ...
      readLayers.each do |key, value|
        if(value.size > 0)
          require 'su_citygmlbridges/Export/layers/' << key
    ...
    def writeface(face, transformation, za, tw, actbuildingid, polygonpos, nosurfacetype, readLayers)   
      ...
      begin
        #GML:ID suchen
        polygonID = FHGelsenkirchenBridges::stringDecode(face.get_attribute("Standard attribute", "PolygonID"))
        #gmlid = nil if(gmlid == "")
      ...
      begin
        #RingID suchen
        linearRingID = FHGelsenkirchenBridges::stringDecode(face.get_attribute("Standard attribute", "LinearRingID"))
      ...
      surfaceid = FHGelsenkirchenBridges::stringDecode(face.get_attribute("Standard attribute", "BoundarySurfaceType ID"))
      if(@gmlType == 0 and @lodNum > 1) # MultiSurface + Lod größer als 1 ## doplneny pozadovany typ
        readLayers.each do |key , value|
          if(key == layer)
