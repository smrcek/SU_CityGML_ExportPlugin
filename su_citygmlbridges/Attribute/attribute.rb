...
module FHGelsenkirchenBridges
  class AttributeEdit
    ...
    def OnImportStandardAttributes(dialog, params)
      ...
      @stdDict.each do |k,v|
        ...
        script << '"' << FHGelsenkirchenBridges::stringDecode(k) << '":"' << FHGelsenkirchenBridges::stringDecode(v) << '"'
      end
      script << "}');";
      @dlg.execute_script(script);
    end
    ...
    def OnAddAttribute(dialog, params)
      ...
      if(p.length == 3)
        if(p[0] == 'std')
          @entity.set_attribute("Standard attribute",FHGelsenkirchenBridges::stringEncode(p[1]),FHGelsenkirchenBridges::stringEncode(p[2]));
        elsif(p[0] == 'gen')
          @entity.set_attribute("Generic attribute",FHGelsenkirchenBridges::stringEncode(p[1]),FHGelsenkirchenBridges::stringEncode(p[2]));
        end
      end
    end
    ...
    def OnRemoveAttribute(dialog, params)
      ...
      if(p.length == 2)
        if(p[0] == 'std')
          @entity.delete_attribute("Standard attribute",FHGelsenkirchenBridges::stringEncode(p[1]));
        elsif(p[0] == 'gen')
          @entity.delete_attribute("Generic attribute",FHGelsenkirchenBridges::stringEncode(p[1]));
        end
      end
    end
  ...
  def FHGelsenkirchenBridges::setid(entities, id)
    entities.each do |e|
  ...
  def FHGelsenkirchenBridges::setsurfacetype(entities, type)
  ...
  def FHGelsenkirchenBridges::setsurfaceid(entities, hash)
    entities.each do |e|
      if(e.class == Sketchup::Face)
  ...
  def FHGelsenkirchenBridges::hasface?(entities)
    entities.each do |e|
      if(e.class == Sketchup::Face)
        return true
      elsif(e.class == Sketchup::Group)
  ...
  def FHGelsenkirchenBridges::findtype(entities, typehash)
  ...
  def FHGelsenkirchenBridges::stringEncode(s)
    if(s == nil)
      return nil
    end
  ...
  def FHGelsenkirchenBridges::stringDecode(s)
    if(s == nil)
      return nil
    end
    coder = HTMLEntities.new
  ...
  def FHGelsenkirchenBridges::str_filter(s)
    if(s == nil)
      return nil
    end
    str = s;
