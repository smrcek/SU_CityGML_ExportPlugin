...
module FHGelsenkirchenTunnels
  class AttributeEdit
    ...
    def OnImportStandardAttributes(dialog, params)
      ...
      @stdDict.each do |k,v|
        ...
        script << '"' << FHGelsenkirchenTunnels::stringDecode(k) << '":"' << FHGelsenkirchenTunnels::stringDecode(v) << '"'
      end
      script << "}');";
      @dlg.execute_script(script);
    end
    ...
    def OnAddAttribute(dialog, params)
      ...
      if(p.length == 3)
        if(p[0] == 'std')
          @entity.set_attribute("Standard attribute",FHGelsenkirchenTunnels::stringEncode(p[1]),FHGelsenkirchenTunnels::stringEncode(p[2]));
        elsif(p[0] == 'gen')
          @entity.set_attribute("Generic attribute",FHGelsenkirchenTunnels::stringEncode(p[1]),FHGelsenkirchenTunnels::stringEncode(p[2]));
        end
      end
    end
    ...
    def OnRemoveAttribute(dialog, params)
      ...
      if(p.length == 2)
        if(p[0] == 'std')
          @entity.delete_attribute("Standard attribute",FHGelsenkirchenTunnels::stringEncode(p[1]));
        elsif(p[0] == 'gen')
          @entity.delete_attribute("Generic attribute",FHGelsenkirchenTunnels::stringEncode(p[1]));
        end
      end
    end
  ...
  def FHGelsenkirchenTunnels::setid(entities, id)
    entities.each do |e|
  ...
  def FHGelsenkirchenTunnels::setsurfacetype(entities, type)
  ...
  def FHGelsenkirchenTunnels::setsurfaceid(entities, hash)
    entities.each do |e|
      if(e.class == Sketchup::Face)
  ...
  def FHGelsenkirchenTunnels::hasface?(entities)
    entities.each do |e|
      if(e.class == Sketchup::Face)
        return true
      elsif(e.class == Sketchup::Group)
  ...
  def FHGelsenkirchenTunnels::findtype(entities, typehash)
  ...
  def FHGelsenkirchenTunnels::stringEncode(s)
    if(s == nil)
      return nil
    end
  ...
  def FHGelsenkirchenTunnels::stringDecode(s)
    if(s == nil)
      return nil
    end
    coder = HTMLEntities.new
  ...
  def FHGelsenkirchenTunnels::str_filter(s)
    if(s == nil)
      return nil
    end
    str = s;
