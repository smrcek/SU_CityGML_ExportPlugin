class IntTunnelInstallation
    ...
    def run
        handle = ""
        @inttunnelinstallations.each_pair {|key, value|
            if(key != "" and key != nil and !@noid)
              handle << "<tun:boundedBy>\n"
              if(@noid or (key == nil and !@generate) )
                handle << "<tun:IntTunnelInstallation>\n"
              else
                handle << "<tun:IntTunnelInstallation gml:id=\"#{key}\">\n"
              end
              ...
              if(!@groupsurfaces)
                ...
                value.each do |v|
                  ...
                  if(@noid or !@generate)
                    ...
                    key = "#{@actbuildingid}_IntTunnelInstallation_#{pos}" if(@generate)
                    pos += 1
                    handle << "<tun:IntTunnelInstallation gml:id=\"#{key}\">\n"
                  end
