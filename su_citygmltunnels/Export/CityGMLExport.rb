require 'su_citygmltunnels\Export\WorldFileLoader'
require 'su_citygmltunnels\Export\GMLExportDialog'
require 'su_citygmltunnels/Attribute/attribute'
require 'su_citygmltunnels\UI\ProgressBar'
...
module FHGelsenkirchenTunnels
  ...
  class CityGMLExport
    ...
    def exportGmlCoreAttributes(dictionaries)
      ...
      if(stdDict != nil)
        ...
        available.each do |key|
          ...
          value = FHGelsenkirchenTunnels::stringDecode(stdDict[key]);
          if(value == nil)
            next;
          end
          case key
    ...
    def exportGenAttributes(dictionaries)
      ...
      if(genDict != nil)
        genDict.each do |k,v|
          key = FHGelsenkirchenTunnels::stringDecode(k);
          value = FHGelsenkirchenTunnels::stringDecode(v);
      ...
      if(stdDict != nil)
        ...
        available.each do |key|
          ...
          case key
          when "measuredHeight"
            @handle << "<tun:measuredHeight uom=\"m\">#{value.to_s()}</tun:measuredHeight>\n"
          when "storeysAboveGround"
            @handle << "<tun:storeysAboveGround>#{value.to_s()}</tun:storeysAboveGround>\n"
          when "storeysBelowGround"
            @handle << "<tun:storeysBelowGround>#{value.to_s()}</tun:storeysBelowGround>\n"
          when "class"
            @handle << "<tun:class>#{value.to_s()}</tun:class>\n"
          when "function"
            @handle << "<tun:function>#{value.to_s()}</tun:function>\n"
          when "usage"
            @handle << "<tun:usage>#{value.to_s()}</tun:usage>\n"
          when "yearOfConstruction"
            @handle << "<tun:yearOfConstruction>#{value.to_s()}</tun:yearOfConstruction>\n"
          when "yearOfDemolition"
            @handle << "<tun:yearOfDemolition>#{value.to_s()}</tun:yearOfDemolition>\n"
          when "storeyHeightsAboveGround"
            @handle << "<tun:storeyHeightsAboveGround uom=\"m\">#{value.to_s()}</tun:storeyHeightsAboveGround>\n"
          when "storeyHeightsBelowGround"
            @handle << "<tun:storeyHeightsBelowGround uom=\"m\">#{value.to_s()}</tun:storeyHeightsBelowGround>\n"
          when "roofType"
            @handle << "<tun:roofType>#{value.to_s()}</tun:roofType>\n"
			when "description"
            @handle << "<gml:description>#{value.to_s()}</gml:description>\n"
          when "name"
            @handle << "<gml:name>#{value.to_s()}</gml:name>\n"
            ...
            if(!isbatch)
              ...
              files.each do |skpN,xmlN|
                if(Sketchup.open_file(skpN))
                  ...
                  @filename_pur = FHGelsenkirchenTunnels::str_filter(@filename_pur)
                  export(xmlFileName)
                end
              end
            end
...
## doplnena funkce dle pozadovaneho typu
      ...
      @handle << "<!-- Exported with Google SketchUp CityGML-Plugin < version #{FHGelsenkirchenTunnels::CITYGML_VERSION} Build #{FHGelsenkirchenTunnels::CITYGML_BUILD} > - #{FHGelsenkirchenTunnels::CITYGML_CREATOR} -->\n"
        ...
        if(entity.class ==Sketchup::Group or entity.class == Sketchup::ComponentInstance)
          ...
          begin
            ...
            t.each do |t2|
              t2.each_pair { | key, value |
                if(key == "id" || key == "ID")
                  buildingid = FHGelsenkirchenTunnels::stringDecode(value.to_s())
                  #ID schon exportiert?
          ...
          if(proceed or nosurfacetype != "")
            ...
            if(buildingid != nil and buildingid != "" )
              if(@exportedid[buildingid] == nil)
                ...
                @handle << "<tun:Tunnel gml:id=\"" + buildingid.to_s() + "\">\n"
                ...
                @handle << "<tun:Tunnel gml:id=\"" + actbuildingid.to_s() + "\">\n"
          ...
          if(stdbuilding == "")
            if(@generate)
              ...
              stdbuilding << "<tun:Tunnel gml:id=\""+ stdbuildingid.to_s()+ "\">\n"
              @exportedid[stdbuildingid.to_s()] = 1
              puts "Building \"#{stdbuildingid}\" exported"
            else
              stdbuilding << "<core:cityObjectMember>\n"
              stdbuilding << "<tun:Tunnel>\n"
            end
          end
    ...
    def writebuffers(lod, nosurfacetype, actbuildingid, readLayers)## doplneny pozadovany typ
      ...
      readLayers.each do |key, value|
        if(value.size > 0)
          require 'su_citygmltunnels/Export/layers/' << key
    ...
    def writeface(face, transformation, za, tw, actbuildingid, polygonpos, nosurfacetype, readLayers)   
      ...
      begin
        #GML:ID suchen
        polygonID = FHGelsenkirchenTunnels::stringDecode(face.get_attribute("Standard attribute", "PolygonID"))
        #gmlid = nil if(gmlid == "")
      ...
      begin
        #RingID suchen
        linearRingID = FHGelsenkirchenTunnels::stringDecode(face.get_attribute("Standard attribute", "LinearRingID"))
      ...
      surfaceid = FHGelsenkirchenTunnels::stringDecode(face.get_attribute("Standard attribute", "BoundarySurfaceType ID"))
      if(@gmlType == 0 and @lodNum > 1) # MultiSurface + Lod größer als 1 ## doplneny pozadovany typ
        readLayers.each do |key , value|
          if(key == layer)
