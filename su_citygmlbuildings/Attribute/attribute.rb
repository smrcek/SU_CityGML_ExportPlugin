...
module FHGelsenkirchenBuildings
  class AttributeEdit
    ...
    def OnImportStandardAttributes(dialog, params)
      ...
      @stdDict.each do |k,v|
        ...
        script << '"' << FHGelsenkirchenBuildings::stringDecode(k) << '":"' << FHGelsenkirchenBuildings::stringDecode(v) << '"'
      end
      script << "}');";
      @dlg.execute_script(script);
    end
    ...
    def OnAddAttribute(dialog, params)
      ...
      if(p.length == 3)
        if(p[0] == 'std')
          @entity.set_attribute("Standard attribute",FHGelsenkirchenBuildings::stringEncode(p[1]),FHGelsenkirchenBuildings::stringEncode(p[2]));
        elsif(p[0] == 'gen')
          @entity.set_attribute("Generic attribute",FHGelsenkirchenBuildings::stringEncode(p[1]),FHGelsenkirchenBuildings::stringEncode(p[2]));
        end
      end
    end
    ...
    def OnRemoveAttribute(dialog, params)
      ...
      if(p.length == 2)
        if(p[0] == 'std')
          @entity.delete_attribute("Standard attribute",FHGelsenkirchenBuildings::stringEncode(p[1]));
        elsif(p[0] == 'gen')
          @entity.delete_attribute("Generic attribute",FHGelsenkirchenBuildings::stringEncode(p[1]));
        end
      end
    end
  ...
  def FHGelsenkirchenBuildings::setid(entities, id)
    entities.each do |e|
  ...
  def FHGelsenkirchenBuildings::setsurfacetype(entities, type)
  ...
  def FHGelsenkirchenBuildings::setsurfaceid(entities, hash)
    entities.each do |e|
      if(e.class == Sketchup::Face)
  ...
  def FHGelsenkirchenBuildings::hasface?(entities)
    entities.each do |e|
      if(e.class == Sketchup::Face)
        return true
      elsif(e.class == Sketchup::Group)
  ...
  def FHGelsenkirchenBuildings::findtype(entities, typehash)
  ...
  def FHGelsenkirchenBuildings::stringEncode(s)
    if(s == nil)
      return nil
    end
  ...
  def FHGelsenkirchenBuildings::stringDecode(s)
    if(s == nil)
      return nil
    end
    coder = HTMLEntities.new
  ...
  def FHGelsenkirchenBuildings::str_filter(s)
    if(s == nil)
      return nil
    end
    str = s;
