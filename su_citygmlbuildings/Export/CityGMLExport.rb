require 'su_citygmlbuildings\Export\WorldFileLoader'
require 'su_citygmlbuildings\Export\GMLExportDialog'
require 'su_citygmlbuildings\Attribute\attribute'
require 'su_citygmlbuildings\UI\ProgressBar'
...
module FHGelsenkirchenBuildings
  ...
  class CityGMLExport
    ...
    def exportGmlCoreAttributes(dictionaries)
      ...
      if(stdDict != nil)
        ...
        available.each do |key|
          ...
          value = FHGelsenkirchenBuildings::stringDecode(stdDict[key]);
          if(value == nil)
            next;
          end
          case key
    ...
    def exportGenAttributes(dictionaries)
      ...
      if(genDict != nil)
        genDict.each do |k,v|
          key = FHGelsenkirchenBuildings::stringDecode(k);
          value = FHGelsenkirchenBuildings::stringDecode(v);
            ...
            if(!isbatch)
              ...
              files.each do |skpN,xmlN|
                if(Sketchup.open_file(skpN))
                  ...
                  @filename_pur = FHGelsenkirchenBuildings::str_filter(@filename_pur)
                  export(xmlFileName)
                end
              end
            end
...
## doplnena funkce dle pozadovaneho typu
      ...
      @handle << "<!-- Exported with Google SketchUp CityGML-Plugin < version #{FHGelsenkirchenBuildings::CITYGML_VERSION} Build #{FHGelsenkirchenBuildings::CITYGML_BUILD} > - #{FHGelsenkirchenBuildings::CITYGML_CREATOR} -->\n"
          ...
          begin
            ...
            t.each do |t2|
              t2.each_pair { | key, value |
                if(key == "id" || key == "ID")
                  buildingid = FHGelsenkirchenBuildings::stringDecode(value.to_s())
                  #ID schon exportiert?
    ...
    def writebuffers(lod, nosurfacetype, actbuildingid, readLayers)## doplneny pozadovany typ
      ...
      readLayers.each do |key, value|
        if(value.size > 0)
          require 'su_citygmlbuildings/Export/layers/' << key
    ...
    def writeface(face, transformation, za, tw, actbuildingid, polygonpos, nosurfacetype, readLayers)   
      ...
      begin
        #GML:ID suchen
        polygonID = FHGelsenkirchenBuildings::stringDecode(face.get_attribute("Standard attribute", "PolygonID"))
        #gmlid = nil if(gmlid == "")
      ...
      begin
        #RingID suchen
        linearRingID = FHGelsenkirchenBuildings::stringDecode(face.get_attribute("Standard attribute", "LinearRingID"))
      ...
      surfaceid = FHGelsenkirchenBuildings::stringDecode(face.get_attribute("Standard attribute", "BoundarySurfaceType ID"))
      if(@gmlType == 0 and @lodNum > 1) # MultiSurface + Lod größer als 1 ## doplneny pozadovany typ
        readLayers.each do |key , value|
          if(key == layer)
